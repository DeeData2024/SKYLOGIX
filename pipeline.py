    ...
    with requests.session()as session:
         ...
         try:
            for city in CITIES:
                ...
                print(f"data fetched for {city} was successful.")
...
def transform(data:list[dict])-> pd.DataFrame:
    ...
    try:
        for response in extract():
            ...
            logging.info(f"transformed data for {record['city']}")
            df = pd.DataFrame(data_records)
            #add date column
            df['Today_date'] = pd.to_datetime('today').normalize()
...
def load(df:pd.DataFrame, table_name:str)-> None:
    try:
         ...
         logging.error(f"Error during loading data to database: {e}")
         print(f"Error during loading data to database: {e}")"""
